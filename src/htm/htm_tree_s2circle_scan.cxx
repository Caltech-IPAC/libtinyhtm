                                        ...
                                        htm_callback callback)
{
    ...
    for (i = 0, count = 0; i < tree->count; ++i) {
      if (htm_v3_distance2(center,reinterpret_cast<T*>
                               ...
                               htm_callback callback)
{
  if(tree->element_types.at(0)==H5::PredType::NATIVE_DOUBLE)
    {
      htm_tree_s2circle_scan_template<double>(tree,center,radius,err,callback);
    }
  else if(tree->element_types.at(0)==H5::PredType::NATIVE_FLOAT)
    {
      htm_tree_s2circle_scan_template<float>(tree,center,radius,err,callback);
    }
  else
    {
      if(err!=nullptr)
