...
template<class T>
void ext_sort(const std::string &file,
              ...
              size_t nitems)
{
  ...
  if (nblk < 2) {
    std::cout << "Skipping multi-way merge step ("
              << file
              << " already sorted)\n";
    return;
  }
  std::cout << "Multi-way external merge sort of "
            << file << "\n";
  ...
  for (mp = 0; mp < nmp; ++mp, sortsz *= mem.k) {
    ...
    if (infd == -1) {
      throw std::runtime_error("failed to open file " + inf + " for reading");
    }
    if (filesz % pagesz != 0) {
  ...
  if (nmp % 2 == 1) {
    if (rename(scratch.c_str(), file.c_str()) != 0) {
      throw std::runtime_error("failed to rename file " + std::string(scratch)
                               + " to " + std::string(file));
    }
  } else {
    if (unlink(scratch.c_str()) != 0) {
      throw std::runtime_error("failed to delete file " + std::string(scratch));
    }
  }
  std::chrono::milliseconds
    msecs(std::chrono::duration_cast<std::chrono::milliseconds>
