...
void add_node(mem_node * const root,
              tree_gen_context &ctx,
              int64_t htmid,
              int64_t count,
              int64_t index);
...
template<class T>
size_t tree_gen(const std::string &datafile,
                ...
                const size_t npoints)
{
  ...
  if (madvise(behind, mapsz, MADV_SEQUENTIAL) != 0)
    {
      throw std::runtime_error("madvise() failed on mmap for file " + datafile);
    }
  data = (const T*) behind;
  ...
  std::cout << "\t"
            << now() - t
            << " sec total (" << i << " tree nodes, "
            << (nseg*ARENA_SEGSZ)/(1024*1024)
            << " MiB memory)\n\n";
  return i;
}
