...
struct blk_writer
{
  ...
  void issue_helper()
  {
    ...
    if (state == BK_WRITE_ERR) {
      ...
      throw std::runtime_error("background thread failed to write() disk block");
    }
    ...
    if (state == BK_WRITE_ERR)
      {
        pthread_mutex_unlock(&mtx);
        throw
          std::runtime_error("background thread failed to write() disk block");
      }
...
template<class T>
void blk_writer_issue(blk_writer<T,true> &b)
{
