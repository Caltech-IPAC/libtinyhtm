  ...
  try
    {
      ...
      compound.insertMember("y", sizeof(typename T::vector_type), vector_type);
      compound.insertMember("z", 2*sizeof(typename T::vector_type), vector_type);

      for(size_t i=0; i<num_data_elements; ++i)
        compound.insertMember(T::names[i],
                              3*sizeof(typename T::vector_type)+data_offset[i],
                              T::types[i]);

      H5::DataSet dataset(file.createDataSet("data",compound,file_space));
      std::vector<T> ra_dec(std::min(npoints,mem.memsz/sizeof(T)));
      std::vector<htm_entry<T> >
        htm_data(std::min(npoints,mem.memsz/sizeof(htm_entry<T>)));
          ...
          for (size_t i=0; i<infile.gcount()/sizeof(T); ++i)
            {
              ...
              for(size_t j=0; j<num_data_elements; ++j)
                memcpy(htm_data[i].data + data_offset[j],
                       ra_dec[i].data(j),data_sizes[j]);
            }
