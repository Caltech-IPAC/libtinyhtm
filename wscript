...
def options(ctx):
    ctx.load('compiler_c compiler_cxx CCfits hdf5_cxx boost')
...
def configure(ctx):
    ...
    # valgrind.
    ctx.env.append_value('CFLAGS', '-DFAST_ALLOC=1')
    ctx.env.append_value('CXXFLAGS', '-DFAST_ALLOC=1')
...
def build(ctx):
    ...
    ctx.stlib(
        ...
        includes='include include/tinyhtm',
        target='sort_and_index',
        name='sort_and_index_st',
        install_path=os.path.join(ctx.env.PREFIX, 'lib'),
        use='M hdf5_cxx'
    )
    ...
    ctx.program(
        ...
        install_path=os.path.join(ctx.env.PREFIX, 'bin'),
        use='M PTHREAD tinyhtm_st hdf5_cxx sort_and_index_st boost'
    )
    ...
    for t in ('htm', 'geometry', 'select'):
        ctx.program(
            ...
            target='test/test_' + t,
            install_path=False,
            use='testobjs M tinyhtm_st'
        )
    # install headers
    inc_dir = ctx.path.find_dir('include')
    ctx.install_files('${PREFIX}/include', inc_dir.ant_glob('**/*'),
                      cwd=inc_dir, relative_trick=True)
    ctx.install_files('${PREFIX}/include/tinyhtm', ctx.path.ant_glob('**/config.h'))
